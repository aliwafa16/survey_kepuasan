...
@section('content')
     <div class="bg-[#0075FF]/14 w-full rounded-md py-2 px-4">
        ...
        <div
            ...
            <button href="#" data-modal-target="addLevel1hModal" data-modal-toggle="addLevel1hModal" type="button"
            ...
            <a href="{{ route('master_data.export_value_level1') }}"
                        ...
                        <td class="px-4 py-2 font-medium text-gray-600 whitespace-nowrap">
                            <div class="flex justify-center gap-1">
                                ...
                                <button class="bg-yellow-500 text-white px-6 py-1 rounded-md" type="button"
                                    data-modal-target="editLevel1Modal" data-modal-toggle="editLevel1Modal"
                                    onclick="openEditModal('{{ $user->f_id }}', '{{ $user->f_position_desc }}', '{{ $user->f_token }}')">Edit</button>
    ...
    {{-- Add modal --}}
    <div id="addLevel1hModal" tabindex="-1" aria-hidden="true"
        ...
        <div class="relative p-4 w-full max-w-xl h-full md:h-auto">
            ...
            <div class="relative p-4 bg-white rounded-lg shadow  sm:p-5">
                ...
                <div class="flex justify-between mb-4 rounded-t sm:mb-5">
                    ...
                    <div>
                        <button type="button"
                            ...
                            data-modal-toggle="addLevel1hModal">
                ...
                <div class="">
                    <form action="{{ route('master_data.level1.store') }}" method="POST">
                        @csrf
                        @method('POST')
                        <div class="mb-3">
                            <label for="f_position_desc" class="block mb-2 text-sm font-medium text-gray-900">Level
                                1</label>
                        ...
                        {{-- <div class="mb-3">
                            <label for="f_token" class="block mb-2 text-sm font-medium text-gray-900">Kuota Link</label>
                            <input type="text" id="text" aria-describedby="helper-text-explanation"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                placeholder="" name="f_token" id="f_token" value="{{ old('ec') }}">
                        </div> --}}
                        <button type="submit"
    ...
    <div id="editLevel1Modal" tabindex="-1" aria-hidden="true"
        class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-modal md:h-full">
        <div class="relative p-4 w-full max-w-xl h-full md:h-auto">
            <!-- Modal content -->
            <div class="relative p-4 bg-white rounded-lg shadow  sm:p-5">
                ...
                <div class="">
                    <form action="{{ route('master_data.level1.update') }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        {{-- <div class="mb-3">
                            ...
                            <input type="text" aria-describedby="helper-text-explanation"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                placeholder="" name="f_token" id="f_token">
                        </div> --}}
    ...
    <div id="importModal" tabindex="-1" aria-hidden="true"
        ...
        <div class="relative p-4 w-full max-w-xl h-full md:h-auto">
            ...
            <div class="relative p-4 bg-white rounded-lg shadow  sm:p-5">
                ...
                <div class="">
                    <form action="{{ route('master_data.level1.import') }}" method="POST"
                    ...
                    <div class="mt-4">
                        ...
                        <a href="{{ route('master_data.level1.export') }}" class="font-normal text-blue-500">Download
                            format</a>
                    </div>
    ...
    <script>
        function openEditModal(id, f_position_desc, f_token) {
            $('#editLevel1Modal #id_f_position_desc').val(id)
            $('#editLevel1Modal #f_position_desc').val(f_position_desc)
            $('#editLevel1Modal #f_token').val(f_token)
        ...
        function hapus_data(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: `{{ url('/master_data/level1_hapus/') }}` + '/' +
