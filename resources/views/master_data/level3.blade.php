...
@section('content')
    <div class="bg-[#0075FF]/14 w-full rounded-md py-2 px-4">
        ...
        <div
            ...
            <button href="#" data-modal-target="addLevel3Modal" data-modal-toggle="addLevel3Modal"
                type="button"
           ...
           <path stroke-linecap="round" stroke-linejoin="round"
            ...
            <a href="{{ route('master_data.export_value_level3') }}" type="button"
        ...
        <table id="search-table">
            ...
            <tbody>
                @foreach ($level3 as $key => $user)
                    <tr>

                        <td>{{ $key + 1 }}</td>
                        <td>{{ $user->relasi_level2->f_position_desc }}</td>
                        ...
                        <td class="px-4 py-2 font-medium text-gray-600 whitespace-nowrap">
                            <div class="flex justify-center gap-1">
                                ...
                                <button class="bg-yellow-500 text-white px-6 py-1 rounded-md" type="button"
                                    data-modal-target="editLevel3Modal" data-modal-toggle="editLevel3Modal"
                                    onclick="openEditModal('{{ $user->f_id }}', '{{ $user->f_position_desc }}', '{{ $user->f_id2 }}')">Edit</button>
    ...
    {{-- Add modal --}}
    <div id="addLevel3Modal" tabindex="-1" aria-hidden="true"
        ...
        <div class="relative p-4 w-full max-w-xl h-full md:h-auto">
            ...
            <div class="relative p-4 bg-white rounded-lg shadow  sm:p-5">
                ...
                <div class="flex justify-between mb-4 rounded-t sm:mb-5">
                    ...
                    <div>
                        <button type="button"
                            ...
                            data-modal-toggle="addLevel3Modal">
                ...
                <div class="">
                    <form action="{{ route('master_data.level3.store') }}" method="POST">
                        @csrf
                        @method('POST')

                        <div class="mb-3">
                            <select class="select select-bordered w-full rounded-lg text-gray-600" id="f_id2" name="f_id2">
                                <option class="text-gray-600" selected value="">-- Pilih level 2 --</option>
                                @foreach($level2 as $key => $value)
                                <option class="text-gray-600" value="{{ $value->f_id }}"> {{ $value->f_position_desc }}
                            </option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="mb-3">
                            <label for="f_position_desc" class="block mb-2 text-sm font-medium text-gray-900">Level 3</label>
    ...
    <div id="editLevel3Modal" tabindex="-1" aria-hidden="true"
        class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-modal md:h-full">
        <div class="relative p-4 w-full max-w-xl h-full md:h-auto">
            <!-- Modal content -->
            <div class="relative p-4 bg-white rounded-lg shadow  sm:p-5">
                ...
                <div class="">
                    <form action="{{ route('master_data.level3.update') }}" method="POST">
                        @csrf
                        @method('PUT')
    ...
    <div class="relative p-4 w-full max-w-xl h-full md:h-auto">
        ...
        <div class="relative p-4 bg-white rounded-lg shadow  sm:p-5">
            ...
            <div class="">
                <form action="{{ route('master_data.level3.import') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('POST')
                    <div class="mb-3">
                ...
                <a href="{{ route('master_data.level3.export') }}" class="font-normal text-blue-500">Download format</a>
            </div>
    ...
    <script>
        function openEditModal(id, f_position_desc, f_id2) {
            $('#editLevel3Modal #id_f_position_desc').val(id)
            $('#editLevel3Modal #f_position_desc').val(f_position_desc)
            $('#editLevel3Modal #f_id2').val(f_id2).trigger('change')
        ...
        function hapus_data(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: `{{ url('/master_data/level3_hapus/') }}` + '/' +
