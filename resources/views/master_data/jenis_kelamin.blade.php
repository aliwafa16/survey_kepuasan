...
@section('content')
    <div class="bg-[#0075FF]/14 w-full rounded-md py-2 px-4">
        ...
        <div
           ...
           <path stroke-linecap="round" stroke-linejoin="round"
            ...
            <a href="{{ route('master_data.export_value_jenis_kelamin') }}"
                        ...
                        <td class="px-4 py-2 font-medium text-gray-600 whitespace-nowrap">
                            <div class="flex justify-center gap-1">
                                <button class="bg-red-500 text-white px-6 py-1 rounded-md"
                                    onclick="hapus_data(`{{ $user->f_gender_id }}`)">Hapus</button>
                                <button class="bg-yellow-500 text-white px-6 py-1 rounded-md" type="button"
                                    ...
                                    onclick="openEditModal('{{ $user->f_gender_id }}', '{{ $user->f_gender_name }}')">Edit</button>
    ...
    <div id="addModal" tabindex="-1" aria-hidden="true"
        ...
        <div class="relative p-4 w-full max-w-xl h-full md:h-auto">
            ...
            <div class="relative p-4 bg-white rounded-lg shadow  sm:p-5">
                ...
                <div class="">
                    <form action="{{ route('master_data.jenis_kelamin.store') }}" method="POST">
                        @csrf
                        @method('POST')
                        <div class="mb-3">
                            <label for="f_gender_name" class="block mb-2 text-sm font-medium text-gray-900">jenis kelamin</label>
                            <input type="text" id="text" aria-describedby="helper-text-explanation"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                placeholder="" name="f_gender_name" id="f_gender_name" value="{{ old('ec') }}">
                        </div>
    ...
    <div id="editModal" tabindex="-1" aria-hidden="true"
        ...
        <div class="relative p-4 w-full max-w-xl h-full md:h-auto">
            ...
            <div class="relative p-4 bg-white rounded-lg shadow  sm:p-5">
                ...
                <div class="">
                    <form action="{{ route('master_data.jenis_kelamin.update') }}" method="POST">
                        @csrf
                        @method('PUT')
                        <input type="hidden" name="id_f_gender_name" id="id_f_gender_name">
                        <div class="mb-3">
                            <label for="f_gender_name" class="block mb-2 text-sm font-medium text-gray-900">jenis kelamin</label>
                            <input type="text" id="f_gender_name" aria-describedby="helper-text-explanation"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                placeholder="" name="f_gender_name" id="f_gender_name" value="{{ old('ec') }}">
                        </div>
    ...
    <div class="relative p-4 w-full max-w-xl h-full md:h-auto">
        ...
        <div class="relative p-4 bg-white rounded-lg shadow  sm:p-5">
            ...
            <div class="">
                <form action="{{ route('master_data.jenis_kelamin.import') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('POST')
                    <div class="mb-3">
                        <label for="file" class="block mb-2 text-sm font-medium text-gray-900">Jenis kelamin</label>
                ...
                <a href="{{ route('master_data.jenis_kelamin.export') }}" class="font-normal text-blue-500">Download format</a>
            </div>
    ...
    <script>
        function openEditModal(id, f_gender_name) {
            $('#editModal #id_f_gender_name').val(id)
            $('#editModal #f_gender_name').val(f_gender_name)
        }
        ...
        function hapus_data(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: `{{ url('/master_data/jenis_kelamin_hapus/') }}` + '/' +
