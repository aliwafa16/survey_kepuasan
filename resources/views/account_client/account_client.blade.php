        ...
        <table id="search-table">
            <thead>
                <tr>
                    ...
                    <th class="text-center">Responden / Token</th>
                    <th class="text-center">Action</th>
                </tr>
            </thead>
            <tbody>
                @foreach ($account_client as $key => $user)
                    <tr>
                        ...
                        <td>
                            <div class="flex flex-col">
                                ...
                                <div>
                                    {{ $user->f_account_noacc }}
                                </div>
                            </div>
                        </td>
                        <td>{{ $user->f_account_email }}</td>
                        <td>{{ $user->f_account_contact }}</td>
                        <td>{{ $user->f_account_token }}</td>
                        <td class="px-4 py-2 font-medium text-gray-600 whitespace-nowrap">
                            <div class="flex justify-center gap-1">
                                <a href="#" class="bg-red-500 text-white px-6 py-1 rounded-md"
                                    data-url="{{ route('account.destroy', $user->f_account_id) }}"
                                    onclick="hapusData(this)">
                                    Hapus
                                </a>
                                <a href="{{ route('account.edit', $user->f_account_id) }}"
