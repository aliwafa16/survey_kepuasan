...
use App\Exports\Level6Export;
use App\Imports\Level6Import;
...
class Level6Controller extends Controller
{
    public function index()
    {
        $data = [
            'title' => 'Data Level 6',
            'btn_add' => 'Tambah Level 6'
        ];
        $data['level6'] = Level6::where('f_account_id', Auth::user()->f_account_id)->with('relasi_level5')->get();
        $data['level5'] = Level5::where('f_account_id', Auth::user()->f_account_id)->get();

        return view('master_data.level6', $data);
    }
    ...
    {
        $data = Level6::where('f_id', $id)->first();
    ...
    public function update(Request $request)
    {
        ...
        $data = Level6::find($request->input('id_f_position_desc'));

        if ($data) {
            $data->f_position_desc = $validatedData['f_position_desc'];
            $data->f_id5 = $validatedData['f_id5'];
            $data->save();
    ...
    {
        return Excel::download(new Level6Export, 'template_level6.xlsx');
    }
    ...
    public function import(Request $request)
    {
        ...
        Excel::import(new Level6Import, $request->file('file'));
    ...
    {
        // Ambil semua data
        $data = Level6::where('f_account_id', Auth::user()->f_account_id)->get();
