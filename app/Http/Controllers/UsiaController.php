...
class UsiaController extends Controller
{
    public function index()
    {
        ...
        $data['data'] = Usia::where('f_account_id', Auth::user()->f_account_id)->get();
        return view('master_data.usia', $data);
    }
    ...
    public function store(Request $request)
    {
        ...
        $request->validate([
            'f_age_desc' => 'required|string'
        ]);

        $data = Usia::create([
            'f_age_desc' => $request->input('f_age_desc'),
    ...
    public function update(Request $request)
    {
        $validatedData = $request->validate([
            'f_age_desc' => 'required|string|max:255',
        ]);

        $data = Usia::find($request->input('id_f_age_desc'));

        if ($data) {
            $data->f_age_desc = $validatedData['f_age_desc'];
            $data->save();
    ...
    public function export(){
        return Excel::download(new UsiaExport, 'template_usia.xlsx');
    }
...
{
    // Ambil semua data
    $data = Usia::where('f_account_id', Auth::user()->f_account_id)->get();
