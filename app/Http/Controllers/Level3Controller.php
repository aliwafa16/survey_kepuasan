...
use App\Exports\Level3Export;
use App\Imports\Level3Import;
...
class Level3Controller extends Controller
{
    public function index()
    {
        $data = [
            'title' => 'Data Level 3',
            'btn_add' => 'Tambah Level 3'
        ];
        $data['level3'] = Level3::where('f_account_id', Auth::user()->f_account_id)->with('relasi_level2')->get();
        $data['level2'] = Level2::where('f_account_id', Auth::user()->f_account_id)->get();

        return view('master_data.level3', $data);
    }
    ...
    {
        $data = Level3::where('f_id', $id)->first();
    ...
    public function update(Request $request)
    {
        ...
        $data = Level3::find($request->input('id_f_position_desc'));

        if ($data) {
            $data->f_position_desc = $validatedData['f_position_desc'];
            $data->f_id2 = $validatedData['f_id2'];
            $data->save();
    ...
    {
        return Excel::download(new Level3Export, 'template_level3.xlsx');
    }
    ...
    public function import(Request $request)
    {
        ...
        Excel::import(new Level3Import, $request->file('file'));
...
{
    // Ambil semua data
    $data = Level3::where('f_account_id', Auth::user()->f_account_id)->get();
