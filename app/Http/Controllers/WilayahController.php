...
class WilayahController extends Controller
{
    public function index()
    {
        $data = [
            'title' => 'Data Wilayah',
            'btn_add' => 'Tambah Wilayah'
        ];
        $data['data'] = Wilayah::where('f_account_id', Auth::user()->f_account_id)->get();
        return view('master_data.wilayah', $data);
    }
    ...
    public function store(Request $request)
    {
        ...
        $request->validate([
            'f_region_name' => 'required|string'
        ]);

        $data = Wilayah::create([
            'f_region_name' => $request->input('f_region_name'),
    ...
    {
        $data = Wilayah::where('f_id', $id)->first();
    ...
    public function update(Request $request)
    {
        $validatedData = $request->validate([
            'f_region_name' => 'required|string|max:255',
        ]);

        $data = Wilayah::find($request->input('id_f_region_name'));

        if ($data) {
            $data->f_region_name = $validatedData['f_region_name'];
            $data->save();
    ...
    public function export(){
        return Excel::download(new WilayahExport, 'template_wilayah.xlsx');
    }
...
{
    // Ambil semua data
    $data = Wilayah::where('f_account_id', Auth::user()->f_account_id)->get();
